...
const ListQuarterlyMapBook = () => {
  ...
  const page = Number(router.query['quarterlyMapPage']) || 1;
  const url = `/input-data/map?quarterlyMapPage=${page}&tabs=quarterly`;
  const [searchQuery, setSearchQuery] = useDebouncedState<string>('', 500);
  const { t } = useTranslation('default');
  const [isOpenDeleteConfirmation, setIsOpenDeleteConfirmation] =
    React.useState<boolean>(false);

  const [quarterlyMapSearchTerm, setQuarterlyMapSearchTerm] =
  ...
  const handleSetPage = (page: number) => {
    const urlSet = `/input-data/production/map?quarterlyMapPage=${page}&tabs=barging`;
  ...
  const filter = React.useMemo(() => {
    ...
    const yearItem = globalSelectNative({
      ...
      searchable: true,
      data: [],
      onSearchChange: setQuarterlyMapSearchTerm,
      searchValue: quarterlyMapSearchTerm,
      onChange: () => {
        router.push(url, undefined, { shallow: true });
      },
    });
