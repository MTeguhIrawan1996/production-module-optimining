...
import { downloadHEProductionValidation } from '@/utils/form-validation/heavy-equipment-production/heavy-equipment-production-validation';
...
interface IDownloadButtonHeavyEquipmentProdProps
  ...
  > {
  defaultValuesState?: Partial<IDownloadHEProductionValues>;
}

export default function DownloadButtonHeavyEquipmentProd({
  defaultValuesState,
  ...rest
}: IDownloadButtonHeavyEquipmentProdProps) {
  ...
  const defaultValues: IDownloadHEProductionValues = {
    period: 'DATE_RANGE',
    startDate: null,
    endDate: null,
    year: null,
  ...
  const methods = useForm<IDownloadHEProductionValues>({
    resolver: zodResolver(downloadHEProductionValidation),
  ...
  const [executeCreate, { loading }] = useDownloadTask({
    onCompleted: ({ createDownloadTasks }) => {
      ...
      sendGAEvent({
        ...
        params: {
          category: 'Produksi',
          subCategory: `Produksi - Alat Berat - Download Data`,
          subSubCategory: '',
      ...
      notifications.show({
        ...
        message: `Data alat berat sedang diproses`,
        icon: <IconCheck />,
      });
    ...
    onError: (error) => {
      if (error.graphQLErrors) {
        const errorArry =
          errorBadRequestField<IDownloadHEProductionValues>(error);
        if (errorArry.length) {
  ...
  const handleSubmitForm: SubmitHandler<IDownloadHEProductionValues> = async (
    data
  ) => {
    ...
    await executeCreate({
      variables: {
        ...
        columnFilter: {
          ...
          companyHeavyEquipmentId: data.companyHeavyEquipmentId || undefined,
        },
      },
    });
  };
