...
import { downloadSampleHouseLabValidation } from '@/utils/form-validation/sample-house-lab/sample-house-lab-mutation-validation';
...
interface IDownloadButtonSampleHouseLabProps
  ...
  > {
  defaultValuesState?: Partial<IDownloadSampleHouseLabValues>;
}

export default function DownloadButtonSampleHouseLab({
  defaultValuesState,
  ...rest
}: IDownloadButtonSampleHouseLabProps) {
  ...
  const defaultValues: IDownloadSampleHouseLabValues = {
    period: 'DATE_RANGE',
    startDate: null,
    endDate: null,
    year: null,
  ...
  const methods = useForm<IDownloadSampleHouseLabValues>({
    resolver: zodResolver(downloadSampleHouseLabValidation),
  ...
  const [executeCreate, { loading }] = useDownloadTask({
    onCompleted: ({ createDownloadTasks }) => {
      ...
      sendGAEvent({
        ...
        params: {
          category: 'Produksi',
          subCategory: `Produksi - Rumah Sampel Dan Lab - Download Data`,
          subSubCategory: '',
      ...
      notifications.show({
        ...
        message: `Data rumah sampel dan lab sedang diproses`,
        icon: <IconCheck />,
      });
    ...
    onError: (error) => {
      if (error.graphQLErrors) {
        const errorArry =
          errorBadRequestField<IDownloadSampleHouseLabValues>(error);
        if (errorArry.length) {
  ...
  const handleSubmitForm: SubmitHandler<IDownloadSampleHouseLabValues> = async (
    data
  ) => {
    ...
    await executeCreate({
      variables: {
        ...
        columnFilter: {
          sampleTypeId: data.sampleTypeId || undefined,
