...
import {
  IDownloadStockpileMonitoringValues,
  useDownloadTask,
...
import { downloadStockpileMonitoringValidation } from '@/utils/form-validation/stockpile-monitoring/stockpile-monitoring-validation';
...
interface IDownloadButtonStockpileMonitoringProps
  extends Omit<
    IDownloadDataButtonProps,
    'methods' | 'submitForm' | 'fields' | 'isDibaledDownload'
  > {
  defaultValuesState?: Partial<IDownloadStockpileMonitoringValues>;
}

export default function DownloadButtonStockpileMonitoring({
  defaultValuesState,
  ...rest
}: IDownloadButtonStockpileMonitoringProps) {
  ...
  const defaultValues: IDownloadStockpileMonitoringValues = {
    period: 'DATE_RANGE',
    startDate: null,
    endDate: null,
    year: null,
  ...
  const methods = useForm<IDownloadStockpileMonitoringValues>({
    resolver: zodResolver(downloadStockpileMonitoringValidation),
  ...
  const [executeCreate, { loading }] = useDownloadTask({
    onCompleted: ({ createDownloadTasks }) => {
      ...
      sendGAEvent({
        ...
        params: {
          category: 'Produksi',
          subCategory: `Produksi - Stockpile Monitoring - Download Data`,
          subSubCategory: '',
      ...
      notifications.show({
        ...
        message: `Data stockpile monitoring sedang diproses`,
        icon: <IconCheck />,
      });
    ...
    onError: (error) => {
      if (error.graphQLErrors) {
        const errorArry =
          errorBadRequestField<IDownloadStockpileMonitoringValues>(error);
        if (errorArry.length) {
